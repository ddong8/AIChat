        ...
        await fetchEventSource(url, {
          ...
          async onopen(openResponse) {
            ...
            try {
              ...
              error = new Error(
                `Failed to send message. HTTP ${openResponse.status} - ${body}`
              );
              ...
              error =
                error ||
                new Error(
                  `Failed to send message. HTTP ${openResponse.status}`
                );
            }
            throw error;
          },
          onclose() {
            ...
            if (!done) {
              if (!lastEvent) {
                reject(
                  new Error(
                    "Server closed the connection unexpectedly. Please make sure you are using a valid access token."
                  )
                );
                return;
              }
          ...
          onmessage(eventMessage) {
            ...
            try {
              ...
              const lastMessage = lastEvent
                ? lastEvent.message.content.parts[0]
                : "";
